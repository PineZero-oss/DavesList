...
if (!empty($cart)) {
    foreach ($cart as $book_id => $qty) {
        ...
        if ($row = $res->fetch_assoc()) {
            $lineTotal = $row['bookPrice'] * intval($qty);
            $orders[] = [
                'id' => $book_id,
                'name' => $row['bookName'],
                'price' => $row['bookPrice'],
                'qty' => intval($qty),
                'total' => $lineTotal,
                'image' => 'uploads/' . $row['bookImage'],
                'status' => 'Processing',
                'date' => date('Y-m-d')
            ];
            $totalItems += intval($qty);
...
<head>
        ...
        <style>
                ...
                .order-card{border-radius:var(--card-radius);overflow:hidden;box-shadow:0 6px 18px rgba(16,24,40,.06)}
                .order-img{height:180px;object-fit:cover}
                .badge-status{font-size:.75rem;padding:.45em .6em;border-radius:.5rem}
        ...
        <?php if ($hasOrder): ?>
            <?php foreach ($orders as $order): ?>
                <?php
                    ...
                    if ($status === 'processing') $statusClass = 'bg-info';
                    elseif ($status === 'shipped') $statusClass = 'bg-primary';
                ...
                <div class="col-12 col-md-6 col-lg-4 order-card-item" data-status="<?= htmlspecialchars($status) ?>">
                    <div class="card order-card h-100 shadow-lg">
                        ...
                        <div class="card-body d-flex flex-column">
                            ...
                            <div class="mb-2">
                                <span class="badge <?= $statusClass ?> badge-status"><?= htmlspecialchars($order['status']) ?></span>
                            ...
                            <div class="mt-auto d-flex justify-content-between align-items-center">
                                ...
                                <div class="text-end">
                                    <a href="#" class="btn btn-sm btn-outline-primary me-2" data-bs-toggle="modal" data-bs-target="#detailModal">Details</a>
                                    <a href="#" class="btn btn-sm btn-primary">Receipt</a>
                                </div>
                            </div>
                        </div>
                    </div>
                ...
                <div class="modal-body">
                    <div class="row g-3">
                        <div class="col-12 col-md-4">
                            <img src="<?= htmlspecialchars($order['image']) ?>" class="img-fluid rounded" alt="cover">
                        </div>
                        <div class="col-12 col-md-8">
                            ...
                            <p><strong>Status:</strong> <span class="badge <?= $statusClass ?>"><?= htmlspecialchars($order['status']) ?></span></p>
                            <hr>
